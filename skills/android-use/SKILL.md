...
- If no authorized device is available after an Android tool call, tell the user to connect by either wired USB debugging or wireless debugging. For wireless onboarding, call `android_wireless_pair_qr(action="create")`, show the returned QR code, ask the user to scan it from Android Wireless debugging, then call `android_wireless_pair_qr(action="complete", session_id=...)`.
- For cable-free use, pair each Android 11+ device once with `android_wireless_pair_qr` or `android_wireless_pair`; successful pairs are appended to `ANDROID_USE_WIRELESS_DEVICES` in `~/.config/android-use/env`. Subsequent tool calls can auto reconnect, or use `android_wireless_reconnect` with `all=true` and `start_scrcpy=true` to reconnect every saved wireless device and open one scrcpy window per device.
...
7. Call `android_start_screen_viewer` when the user wants a Codex-openable Android web UI showing Android Use tool action steps with screenshots. Open the returned localhost URL in the Codex browser.
8. For long evidence sessions, keep `android_start_screen_viewer` running while direct Android tools are used; it records screenshot-backed action events into the timeline session directory.
...
- `android_scrcpy_resident_status` reports and starts the resident monitor. The monitor uses the app wrapper for visible scrcpy windows. Set `ANDROID_USE_SCRCPY_RESIDENT_SERIALS` to a comma-separated serial list only when multiple resident windows are wanted, set `ANDROID_USE_SCRCPY_RESIDENT=0` to disable the monitor, or set `ANDROID_USE_SCRCPY_ON_TOOL_CALL=0` to stop tool calls from opening scrcpy.
- `android_wireless_pair_qr` is the preferred first-time wireless onboarding flow for users who do not want to type IP/port/code manually. `action="create"` returns a QR PNG image and a `session_id`; after the user scans it on Android's Wireless debugging screen, call `action="complete"` with that `session_id` to discover the pairing service through `adb mdns`, run `adb pair`, save the device, reconnect, and open scrcpy by default. `android_wireless_pair` remains available for manual pairing code flows. `android_wireless_reconnect` reuses the saved config and refreshes dynamic mDNS ports; pass `all=true` to reconnect all saved wireless devices and pass `start_scrcpy=true` to mirror each one.
...
- `android_start_screen_viewer` starts the local screenshot timeline UI. The returned URL shows only Android Use tool action steps with screenshots; it does not continuously poll the device screen and does not add automatic screen-change entries. Timeline assets are stored under `.screen/timelines/` unless `session_dir` is provided.
- Timeline action screenshots are captured after the Android Use tool action settles, not before it. The default wait is 0.8 seconds and can be tuned with `ANDROID_USE_TIMELINE_AFTER_DELAY_SEC`.
