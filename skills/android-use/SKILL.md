...
- `android_start_video_recording` starts a headless scrcpy `--record` MP4 capture without opening a new visible window. `android_stop_video_recording` stops only that recording process and returns a local MP4 path plus Markdown. Use these tools for real videos; do not use `android_start_recording` for video capture.
