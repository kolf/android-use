...
3. For routine Android operation, keep a visible desktop scrcpy window available. The MCP server starts a resident monitor by default and keeps one visible scrcpy window alive for one physical device, preferring `ANDROID_USE_SCRCPY_RESIDENT_SERIALS`, `ANDROID_USE_SERIAL`, or `ANDROID_SERIAL` when set. `android_agent_run` and `android_agent_step` also ensure that window before executing actions, reuse an existing visible scrcpy process for the same device, and do not start WebRTC.
...
- `android_start_scrcpy` defaults to a visible, draggable video-only scrcpy window with an explicit initial size, `keep_alive=true`, `keyboard="sdk"`, and `prefer_text=true`. The macOS size helper applies the requested size once by default so it does not interfere with keyboard focus; pass `lock_window_continuous=true` only if continuous size enforcement is needed. Pass `audio=true` to forward audio, `keyboard="uhid"` for physical-keyboard behavior, `legacy_paste=true` for paste fallback, or `keep_alive=false` if manually closing the window should not relaunch it.
- `android_scrcpy_resident_status` reports and starts the resident monitor that keeps one visible scrcpy window alive by default. The monitor never starts WebRTC. Set `ANDROID_USE_SCRCPY_RESIDENT_SERIALS` to a comma-separated serial list only when multiple resident windows are wanted, or set `ANDROID_USE_SCRCPY_RESIDENT=0` when an always-on desktop window is not wanted.
...
- `xiaoluxue_open_native_subject` opens the native subject map through the app-only `xlx://router/study/subject` route; use this instead of generic browser or deep-link launching. `xiaoluxue_map_fast_path` controls the native study map without screenshots: select visible indexes like `1.5`, open `题型突破`, `错题`, `笔记本`, `学习任务`, or `薄弱知识`. When `subject_id`/`subject` is present it routes first, and known presets such as `语文 1.5 题型突破` avoid `uiautomator dump` entirely. `xiaoluxue_map_snapshot` reads the current subject/chapter/visible indexes/actions.
